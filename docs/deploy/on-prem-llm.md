...
1. **AWS SSO 프로필 저장과 로그인.** 화면이 `~/.aws/config`를 읽어 이 PC에 있는 SSO
   프로필을 목록으로 보여 줍니다. 하나 고르고(직접 입력해도 됩니다) 저장한 뒤
   `SSO 로그인`을 누르면 서버가 `aws sso login`을 실행하고, AWS CLI가 출력하는 인증
   주소와 코드를 화면에 띄웁니다. 브라우저가 자동으로 열리지 않는 PC에서도 그 주소로
   들어가면 됩니다. 현재 세션이 살아 있는지는 `sts get-caller-identity`로 실제 확인하고,
   언제 만료되는지는 AWS CLI의 토큰 캐시에서 읽어 함께 보여 줍니다.
...
### 로그인이 끝나지 않을 때: 장치 코드 방식

AWS CLI v2.22부터 기본 로그인 방식이 바뀌었습니다. 브라우저에서 승인한 결과가 **이
PC의 로컬 포트로 되돌아와야** 완료되는 방식인데, 다음 경우에는 그 되돌아오는 길이
없어 화면이 계속 "로그인 진행 중"에 머뭅니다.

- 브라우저를 다른 기기에서 열어야 하는 경우 (서버에 설치한 Paperclip에 노트북에서 접속)
- 사내 보안 정책이나 백신이 로컬 콜백을 막는 경우

이럴 때 로그인 버튼 아래의 **장치 코드 방식** 체크박스를 켜고 다시 누르세요.
`aws sso login --use-device-code`로 실행되어, 화면에 뜬 주소와 코드를 아무 기기의
브라우저에서 입력하면 끝납니다. 되돌아올 필요가 없습니다.

<Note>
**Windows에서 "AWS CLI 없음"이 뜨는데 터미널에서는 `aws --version`이 되는 경우**는
거의 항상 Paperclip 서버를 띄운 뒤에 AWS CLI를 설치했기 때문입니다. 서버 프로세스는
시작할 때의 `PATH`를 그대로 들고 있습니다. 서버를 다시 시작하면 해결됩니다.
(MSI 기본 설치 경로와 사용자 단위 설치 경로는 `PATH`에 없어도 직접 찾아보므로,
대개는 이 상황에서도 정상 동작합니다.)
</Note>

<Warning>
**화면 없는 서버(우분투 systemd 배포)에는 Bedrock 레인을 만들지 마세요.** 로그인
자체는 장치 코드 방식으로 가능하지만, 문제는 로그인이 아니라 만료입니다. SSO 세션은
몇 시간마다 끊기고 그때마다 사람이 브라우저에서 다시 승인해야 합니다. 서버에 붙이면
